...
%                  
            ...
            %%%%%%foci_nn=(foci+foci2+foci3_N)/3;
            
            %foci_nn=(foci+foci3_N)/2;
            
            %foci_nn=medfilt1(foci_nn,3);
            
            %%%%%%[fit_s, ~]=createFit_smoothingspline(foci_nn,0.975);
            ...
            for i=2:1:length(foci)-2
                ...
                if aux_g==0
                    ...
                    if (foci_n(i)>0.75) && (foci_n(i)<=2.75) 
                    ...
                    if length(find(foci_n>0.75 & foci_n<=2.75))<4 && foci_n(i)~=0
...
%             
    ...
    if tb1<=1 && tc1<=1 && tb2<=1
            ...
            if tc2>0 && tc4>0 && tc8>8 
                   ...
                   foci4=foci4(aux_fn+tb1+tc1+tb2+tc2:aux_stop-1);
                   
                   foci4=[-1,foci4,NaN*ones(1,(length(foci)-length(foci4)-1))];
    
            end
   
  
        
    elseif tb1<=1 && tc1<=1 && tb2>=1
    
            if tc2>0 && tc4>0 && tc8>0
                
                   aux_stop=find(foci_n>=11,1);
                   
                   foci4=foci4(aux_fn+tb1+tc1+tc2:aux_stop-1);
