...
while time <= numIm
    ...
    %Frames are repeated so this is not useful to segmentation in
    %Morphopmetrics when stray regions are active.
%     %Saves the corrected regions after linking as binary mask for further
%     %use in Morphometrics:    - added by Andres Florez on 04/17/2020
%     mask_name = char(regexp(contents(1).name,['(\w*)t'],'tokens','once'));
%     dataname_mask=[dirname,'mask2err_', mask_name, '.tif']; 
%     disp('BatchSuperSeggerOpti : Saving mask2 as .tif');
