function [segdata,Imag_out,Iseg_out,Im1]=simply_segment_cl(Im0,mask1,fr_num,rect1)
%Tristan Ursell
%April 2013
%
%Several bug fixes, specially segmentation by simple threshold - Andres Florez - 05/06/2020
...
%are cells in proximity?
global v_prox

%reject false positives?
global v_falsepos

%are seeded contours being used
global v_seed
...
%minimum relative variacne required for any segmentation
global f_seg_min
...
%simple threshold
global v_simplethres   %This was added back- Andres Florez 05/06/20
...
%check to make sure there is enough variation in the image to warrant
%segmetation
if max([reg_test_prop.Area])<f_areamin %<---- sets the discrimination for segmentation
    ...
    segdata.num_objs=0;
   disp('Image did not meet the minimum requirements for segmentation.')
...
if or(~v_seed,and(v_seed,fr_num==1))
    ...
    if v_method==2
        ...
        I4=bwmorph(~bwmorph(~I3,'majority'),'spur','Inf');
        
        %remove small areas and big areas, clear border
        I5=bwareaopen(I4,areamin_use,4);
        ...
        da=areamin_use;
        G_fill=bwareaopen(imopen(~bwareaopen(~I5,da,4),strel('disk',2)),areamin_use,4).*mask1;
    end
    %---------------------------------------
    %---------------------------------------
    ...
    %***************************************
    if v_method==3
        if v_simplethres                 % Simple threshold method was fixed - Andres Florez 05/06/20
            I_thres=I1<=f_hmin;
        else
            %extract sub-region
            ...
            I_thres=(bck_mean1-I1)>(f_hmin*bck_std1);
        end
        
        if v_imtype==3
            I_thres=imfill(I_thres,'holes',4);
        end
        
        %apply area constraints to thresheld image
        G_seg=bwareaopen(I_thres,areamin_use,4);
        %G_seg=bwareaopen(I_thres,areamin_use,4)&~bwareaopen(I_thres,areamax_use,4);
        
        %apply mask and fills holes
        %G_fill=imfill(bwmorph(G_seg,'close'),4,'holes').*mask1;
        %G_fill=bwmorph(G_seg,'close').*mask1;
    ...
    if v_method==4
        ...
        G_shed=imfill(I3,'holes');  % Fixed - Andres Florez 05/06/20
        G_seg=bwareaopen(G_shed,areamin_use,4);
        G_fill=G_seg;
    end
    %---------------------------------------
    ...
    Ltemp=bwlabel(bwareaopen(G_fill,4*areamax_use,8)); %This is fixed
    if max(Ltemp(:))==1
        G_fill=G_fill.*(~Ltemp);
    end
    ...
    if and(f_hmin_split>1,v_prox)
        ...
        %put back regions that were too small to resegmented
    ...
    if max(G_fill2(:))>0
        ...
        %int_rej=1;  %ADD IN TEMPORARY ADJUSTMENT TO THIS PARAMETER
        ...
        if v_falsepos
            if v_method==3
                if v_simplethres                     %This was fixed
                Lout=falsepos(G_fill2,I1,f_int_rej);
                else
                    Lout=falsepos(G_fill2,I1,f_int_rej);
                end
        ...
        if max(Lout(:))==0
            disp(['False positive rejection is removing all elements.'])
        end
    ...
    L_final=zeros(size(I1));
    magG_out=mat2gray(magG);
end
...
if max(L_final(:))>0
    ...
    segdata.num_objs=size(C2,1);          %This was fixed based on simply_segment.m - Andres Florez 05/06/20
...
% function Lout=falsepos(Lin,Iin,int_rej,hmin,v_imtype)  %This function
%
% doesnt work well for binary masks therefore I commented it and used the
% falsepos.m file - Andres Florez - 05/06/20
% 
% 
