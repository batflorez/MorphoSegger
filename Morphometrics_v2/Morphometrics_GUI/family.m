...
function cells = family(G_parent,pert_same,pert_child,frame_diff,handles)
...
for i=1:Nregs
    ...
    set(handles.text_process,'String',['Cell lineage: ' num2str(round(100*i/Nregs)) '%']);
    drawnow
...
%{
%future implementation:  direct graph in time
try
    %get forward / backward only checkbox value
    if get(handles.checkbox_forward,'Value');
        G_groups=triu(G_groups);
    end
    
    if get(handles.checkbox_backward,'Value');
        G_groups=tril(G_groups);
    end
catch
end
...
try %attempt regular group analysis (i.e. non-sparse)
    ...
    [conn_groups,~]=graph_analysis_sparse_morphometrics(G_groups,handles,'min_conn',0,'max_link',n_frames);
    ...
    if strcmp(er1.identifier,'MATLAB:nomem')
        ...
        [conn_groups,~]=graph_analysis_morphometrics(full(G_groups),handles,'min_conn',0,'max_link',n_frames);
    else
...
for k=1:length(frame_range)
    i=frame_range(k); %frame number
    
    figure;
    imagesc(imread(G_parent,i))
    hold on
    templist=find(fr==i);
    
    for j=1:length(templist)
        ind1=templist(j);
        plot([Xbox1(ind1),Xbox2(ind1),Xbox2(ind1),Xbox1(ind1),Xbox1(ind1)],[Ybox1(ind1),Ybox1(ind1),Ybox2(ind1),Ybox2(ind1),Ybox1(ind1)],'-','Linewidth',1,'color',[0.5 0.5 0.5])
        text((Xbox1(ind1)+Xbox2(ind1))/2,(Ybox1(ind1)+Ybox2(ind1))/2,['O/' num2str(obj(ind1)) ' E/' num2str(els(ind1))],'color',[0.7 0 0])
    end
    axis equal tight
    colormap(gray)
...
%show images for elements of a group
figure;
for i=1:length(conn_groups);
    grp=i;
    for k=1:conn_groups(grp).num_els
        el1=conn_groups(grp).elements(k);
        fr1=fr(el1);
        obj1=obj(el1);
        
        im_temp=imread(G_parent,fr1);
        I1=double(im_temp)/double(max(im_temp(:)));
        I2=im_temp==obj1;
        ...
        imagesc(B)
        hold on
        plot([Xbox1(el1),Xbox2(el1),Xbox2(el1),Xbox1(el1),Xbox1(el1)],[Ybox1(el1),Ybox1(el1),Ybox2(el1),Ybox2(el1),Ybox1(el1)],'-','Linewidth',1,'color',[0.5 0.5 0.5])
        text((Xbox1(el1)+Xbox2(el1))/2,(Ybox1(el1)+Ybox2(el1))/2,['O' num2str(obj(el1)) ' E' num2str(els(el1)) ' F' num2str(fr1) ' C' num2str(grp)],'color',[0.7 0 0])
        hold off
        
        axis equal tight
        colormap(gray)
        title(['Frame: ' num2str(fr1) ', cell: ' num2str(grp)])
        pause(0.1)
    end
end
close
