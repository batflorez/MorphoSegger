...
function [mesh,branches]=contour2mesh(X,Y,varargin)
...
if and(N>(2*min_d),get(handles.checkbox_cellwidth,'Value'))
...
%[X1,Y1,I1]=polyxpoly(VxL,VyL,Xint,Yint); %requires mapping toolbox
%inds = ceil(I1(:,1)/3);

[X1,Y1,Iout,~] = intersections(VxL,VyL,Xint,Yint,false);
inds=ceil(Iout/3);
