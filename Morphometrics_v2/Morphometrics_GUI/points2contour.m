...
%[Xout,Yout,I] = points2contour(Xin,Yin,P,direction,dlim)
%[Xout,Yout,I,orphans] = points2contour(Xin,Yin,P,direction,dlim)
%
...
%
%The optional output 'I' is the index list such that:
%
% Xout = Xin(I);
% Yout = Yin(I);
%
...
%
%Note that this will *not* neccessarily form the shortest path between all
...
%box on
%
%Example 3:  continous points, pathological case
%N=200;
%P=1;
%theta=linspace(0,2*pi*(1-1/N),N);
%[~,I]=sort(rand(1,N));
...
%box on
%
%Example 4:  continous points, distance limit applied
%N=200;
%P=1;
%theta=linspace(0,2*pi*(1-1/N),N);
%[~,I]=sort(rand(1,N));
...
%
%[Xout,Yout,orphans]=points2contour(Xin,Yin,P,'cw',1);
%
%figure;
%hold on
%plot(Xin,Yin,'b-')
...
if length(Xin)<3
    error('The point list must have three or more elements.')
end
